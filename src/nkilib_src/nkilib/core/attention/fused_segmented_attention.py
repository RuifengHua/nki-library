    ...
    for _ in nl.dynamic_range(0, is_partial_reg):
        if is_kvp:
            ...
            k_prior_sbuf = allocator.alloc_sbuf_tensor(
                ...
                dtype=nl.bfloat16,
                block_dim=[num_k_tiles_per_seg],
                num_free_tiles=[num_k_tiles_per_seg],
                align_to=32,
            )
            v_prior_sbuf = allocator.alloc_sbuf_tensor(
                shape=(_V_TILE_SZ, head_dim),
                dtype=nl.bfloat16,
                block_dim=[num_v_tiles_for_prior],
                num_free_tiles=[num_v_tiles_for_prior],
            )
            load_kv_cache_fn(
                k_cache,
                v_cache,
