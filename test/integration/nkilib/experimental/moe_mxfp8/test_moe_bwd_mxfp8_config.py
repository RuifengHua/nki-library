...
class TestMXFP8MOEBwdConfigDefaults:
    ...
    def test_default_construction(self):
        ...
        assert config.is_tensor_update_accumulating == True
        assert config.skip_grad_initialization == False
        assert config.bias == False

    def test_post_init_creates_phase_configs(self):
        config = MXFP8MOEBwdConfig()
        assert config.phase1_config != None
        assert config.phase2_config != None
        assert config.phase3_config != None
        assert config.phase4_config != None
    ...
    def test_post_init_creates_clamp_limits(self):
        config = MXFP8MOEBwdConfig()
        assert config.clamp_limits != None
        assert isinstance(config.clamp_limits, ClampLimits)
        assert config.clamp_limits.linear_clamp_upper_limit == None
        assert config.clamp_limits.linear_clamp_lower_limit == None
        assert config.clamp_limits.non_linear_clamp_upper_limit == None
        assert config.clamp_limits.non_linear_clamp_lower_limit == None

    def test_skip_dma_remains_none_after_post_init(self):
        config = MXFP8MOEBwdConfig()
        assert config.skip_dma == None
...
class TestMXFP8MOEBwdConfigCustomValues:
    ...
    def test_custom_accumulation_flags(self):
        ...
        assert config.is_tensor_update_accumulating == False
        assert config.skip_grad_initialization == True
    ...
    def test_custom_skip_dma(self):
        ...
        assert config.skip_dma.skip_token == True
        assert config.skip_dma.skip_weight == False
