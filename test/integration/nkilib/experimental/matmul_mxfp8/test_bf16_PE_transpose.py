...
"""Integration tests for load_tile_bf16_PE_transpose.

Validates that load_tile_bf16_PE_transpose produces the correct swizzled SBUF
...
def pe_transpose_load_kernel(
    ...
    for k_idx in nl.affine_range(tiles_in_k):
        ...
        load_tile_bf16_PE_transpose(load_loc, data_store_loc=data_store_loc)
...
@pytest_test_metadata(name="BF16 PE Transpose vs DGT Load")
...
class TestBF16PETranspose:
    """
    Correctness tests for load_tile_bf16_PE_transpose (direct DMA mode).
    """
...
@pytest_test_metadata(name="BF16 PE Transpose Indirect DMA")
@pytest_marks(["bf16_pe_transpose_indirect"])
@pytest.mark.platforms(exclude=[common_dataclasses.Platforms.TRN1, common_dataclasses.Platforms.TRN2])
class TestBF16PETransposeIndirect:
    """
    Correctness tests for load_tile_bf16_PE_transpose with indirect DMA (gather mode).
    """
...
@pytest_test_metadata(name="BF16 PE Transpose Indirect DMA OOB Skip")
@pytest_marks(["bf16_pe_transpose_oob_skip"])
@pytest.mark.platforms(exclude=[common_dataclasses.Platforms.TRN1, common_dataclasses.Platforms.TRN2])
class TestBF16PETransposeOOBSkip:
    """
    Correctness tests for load_tile_bf16_PE_transpose with oob_mode.skip.
...
@pytest_test_metadata(name="BF16 PE Transpose Multi K-Tile")
@pytest_marks(["bf16_pe_transpose_multi_k"])
@pytest.mark.platforms(exclude=[common_dataclasses.Platforms.TRN1, common_dataclasses.Platforms.TRN2])
class TestBF16PETransposeMultiK:
    """
    Correctness tests for load_tile_bf16_PE_transpose with tiles_in_k > 1.
