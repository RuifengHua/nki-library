...
def make_view_3d(shape, tile_size_2d):
    ...
    layout = HBMLayout("mock", 0, strides, "float32", "shared_hbm")
...
class TestBatchDimSkipOver:
    ...
    def test_3d_int_batch_then_int_tile(self):
        ...
        assert child.grid.remaining[1] == 256

    def test_3d_batch_then_ftile(self):
        v = make_view_3d((8, 128, 256), (128, 128))
        child = v[0, 0, 1]
        assert child.grid.remaining[0] == 128
        assert child.grid.remaining[1] == 128
    ...
    def test_2d_no_batch(self):
        ...
        child = v[0, 1]
        assert child.grid.remaining == (128, 256)
...
class TestIndirectDimPreservation:
    ...
    def test_scalar_indirect_after_batch(self):
        ...
        child_b = v[0]
        assert child_b.layout.indirect is None
...
class TestMin2DGuard:
    ...
    def test_1d_tile_on_3d_tensor(self):
        ...
        assert v.ndim == 3
        assert v.grid.tile_size == (4, 8, 64)
        child = v[0, 0]
        assert child.ndim >= 2
...
class TestChainedViewSelect:
    ...
    def test_slab_then_row(self):
        ...
        slab = v[1]
        assert slab.grid.remaining[0] == 4
        assert slab.grid.remaining[1] == 64
        row = slab[2, :]
        assert row.grid.remaining[0] == 1
        assert row.grid.remaining[1] == 64
